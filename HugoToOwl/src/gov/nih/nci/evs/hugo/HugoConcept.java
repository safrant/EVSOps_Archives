...
public class HugoConcept {
	...
	public HugoConcept(HashMap<String, String> propertyValueList) {
	...
	private void separateDelimitedProperties() {
		...
		for (String key : keys) {
			...
			if (HugoToOwl.getDelimitedColumns().containsKey(keyUnderscored)) {
	...
	private void delimitDelimitedProperties() {
		for (String key : delimitedProperties) {
			// String keyUnderscored = key.replace(" ", "_");
			String delimiterName = (String) HugoToOwl.getDelimitedColumns()
					.get(HugoToOwl.underscoredString(key));
			String delimiter = (String) HugoToOwl.getDelimiters().get(
					delimiterName);
			String value = properties.get(key);
			if (value != null && value.length() > 0) {
				Vector<String> delimitedString;
				if (key.contains("Specialist Database")) {
					...
					for (int x = 0; x < result.length; x++)
						...
						delimitedString.add(result[x].trim());
				} else {
					Pattern p = Pattern.compile(delimiter);
					delimitedString = HugoCsvParser.tokenizeString(value, p);
				}
	...
	private void loadSpecialistDatabase() {
		String prefix = (String) HugoToOwl.getSpecialistDatabases().get("name");
		// System.out.println("Start LOAD: " + this.code);
		Set<String> propNames = tokenizedDelimitedProperties.keySet();
		HashMap<String, String> values = new HashMap<String, String>();
		Properties test = HugoToOwl.getSpecialistDatabases();
				...
				for (Integer i = 0; i < propValues.size(); i++) {
					...
					if (value != null && value.trim().length() > 0) {
						...
						if (propName.equals("Specialist Database Links")) {
							this.specialistDatabaseLinks.put((String) HugoToOwl
									.getSpecialistDatabases().getProperty(
											i.toString()), value);
						} else if (propName.equals("Specialist Database IDs")) {
							this.specialistDatabaseIds.put((String) HugoToOwl
									.getSpecialistDatabases().getProperty(
											i.toString()), value);
						} else {
							System.out
